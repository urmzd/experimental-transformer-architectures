...
Usage:
    glassbox benchmark [--versions v1_shared_attn,v8_lowrank_vv] [--minutes 10] [--batch 491520] [--warmup 5] [--seeds 1337,1338,1339]

Protocol notes:
- Warmup steps in the training loop run forward+backward only (no optimizer
  updates), so no training happens outside the wallclock budget, and the token
  stream continues into the timed run without replay.
- Each seed is a full independent run; pass --seeds with several values to get
  the variance plumbing (aggregate with `glassbox results`, which reports mean±std).
"""
...
def main(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        prog="glassbox benchmark",
        description="Benchmark model versions under identical conditions")
