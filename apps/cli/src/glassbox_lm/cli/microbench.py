...
harness is `glassbox benchmark`.

Usage:
    glassbox microbench                                                   # all models, defaults
    glassbox microbench v8_lowrank_vv v9_linattn v10_state_cond_op        # specific models only
    glassbox microbench --seq-len 512 --batch 4 --steps 3                 # custom sizes
"""
...
def benchmark_model(name, make_fn, vocab_size, batch, seq_len, n_iters, device):
    ...
    try:
        ...
        if use_bf16:
            ...
            # Keep small params in fp32 (mirrors the training loop logic)
            with torch.no_grad():
...
def main(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(prog="glassbox microbench", description="Rapid model benchmark")
