...
optional --checkpoint (a state_dict saved by the training loop) and --version.
"""
...
def load_model(version: str, checkpoint: str | None):
    ...
    if checkpoint:
        ...
        sd = sd.get("model", sd)  # training ckpts wrap weights under "model"
...
def main(argv: list[str] | None = None):
    p = argparse.ArgumentParser(prog="glassbox observe", description=__doc__,
                                formatter_class=argparse.RawDescriptionHelpFormatter)
    sub = p.add_subparsers(dest="cmd", required=True)

    def common(sp):
        sp.add_argument("--version", default="v8_lowrank_vv")
        sp.add_argument("--checkpoint", default=None, help="state_dict from a training run")
