...
Usage:
    glassbox data prepare-code [--max-docs 100000] [--val-ratio 0.02]
...
# Artifacts land under ./data in the working directory (matching the defaults
# in glassbox_lm.core.config.DataConfig), not inside the installed package.
ROOT = Path(os.environ.get("GLASSBOX_DATA_DIR", "data")).resolve()
...
def main(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        prog="glassbox data prepare-code",
        description="Prepare code dataset for training")
    ...
    print("  torchrun --standalone --nproc_per_node=3 -m glassbox_lm.training")
